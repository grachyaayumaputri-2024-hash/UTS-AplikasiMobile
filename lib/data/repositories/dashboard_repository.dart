...
class DashboardRepository {
  ...
  Future<List<TicketModel>> getMyAssignedTickets() async {
    final response = await _client.get(
      '/tickets',
      queryParams: {'assigned_to': 'me', 'limit': 10, 'sort': 'updated_at:desc'},
    );
    final List<dynamic> data = response['data'] as List<dynamic>;
    return data
        .map((e) => TicketModel.fromJson(e as Map<String, dynamic>))
  ...
  Future<List<TicketModel>> getUnassignedOpenTickets() async {
    final response = await _client.get(
      '/tickets',
      queryParams: {'assigned_to': 'unassigned', 'status': 'open', 'limit': 10, 'sort': 'created_at:desc'},
    );
    final List<dynamic> data = response['data'] as List<dynamic>;
    return data
        .map((e) => TicketModel.fromJson(e as Map<String, dynamic>))
