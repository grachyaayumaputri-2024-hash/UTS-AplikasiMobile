...
class _TicketDetailScreenState extends State<TicketDetailScreen>
  ...
  AppBar _buildAppBar(
    ...
    // Pengecekan aman: jika panjang ID kurang dari 8 karakter, tampilkan apa adanya.
    final displayId = ticket.id.length >= 8
    ...
    return AppBar(
      title: Column(
        ...
        children: [
          Text(
            '#$displayId', // <--- Sudah aman menggunakan variabel pengaman
...
Future<void> _confirmDelete(BuildContext context, TicketModel ticket) async {
  // 1. Tampilkan Dialog dan simpan hasilnya ke variabel 'confirm'
  final confirm = await showDialog<bool>(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      content: Text(
        'Tiket "${ticket.title}" akan dihapus permanen.',
  ...
  // 2. Cek validasi konfirmasi awal
  if (confirm != true) return;

  // 3. Ambil provider & state SEBELUM proses async (await) berjalan
  final tp = context.read<TicketProvider>();
  final scaffoldMessenger = ScaffoldMessenger.of(context);
  final navigator = Navigator.of(context);

  // 4. Jalankan proses hapus data ke API / Database
  final ok = await tp.deleteTicket(ticket.id);

  // 5. Eksekusi feedback UI menggunakan state yang sudah disimpan di atas
  if (ok) {
    navigator.pop(); // Kembali ke halaman list tiket
    scaffoldMessenger.showSnackBar(
      const SnackBar(
        content: Text('Tiket berhasil dihapus.', style: TextStyle(fontFamily: 'Poppins')),
    ...
    scaffoldMessenger.showSnackBar(
      SnackBar(
        content: Text(
          tp.errorMessage ?? 'Gagal menghapus tiket.',
