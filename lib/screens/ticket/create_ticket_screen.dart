...
class _CreateTicketScreenState extends State<CreateTicketScreen>
  ...
  Future<void> _pickFromCamera() async {
    ...
    if (photo != null) {
      final bytes = await photo.readAsBytes(); // cross-platform, aman di web
      setState(() => _attachments.add(
        PickedAttachment(bytes: bytes, fileName: photo.name),
      ));
    }
  }
...
// _pickFromGallery
  Future<void> _pickFromGallery() async {
    ...
    if (image != null) {
      ...
      setState(() => _attachments.add(
        PickedAttachment(bytes: bytes, fileName: image.name),
      ));
    }
  }

// _pickFile — WAJIB tambah withData: true supaya bytes ke-load di web
  Future<void> _pickFile() async {
    final result = await FilePicker.platform.pickFiles(
      type: FileType.custom,
      allowedExtensions: ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'txt'],
      allowMultiple: false,
      withData: true, // <-- penting! tanpa ini, bytes null di Flutter Web
    );
    if (result != null && result.files.single.bytes != null) {
      final f = result.files.single;
      setState(() => _attachments.add(
        PickedAttachment(bytes: f.bytes!, fileName: f.name),
      ));
    }
  }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                // Info helpdesk yang akan menangani
                if (_selectedCategory != null)
                  _buildHelpdeskInfoBanner(isDark),
        ...
        if (_attachments.isNotEmpty) ...[
          Wrap(
            ...
            children: _attachments.asMap().entries.map((e) {
              ...
              return _buildAttachmentPreview(attachment, isImage, idx, isDark);
            }).toList(),
          ),
          const SizedBox(height: 10),
        ],
  ...
  Widget _buildAttachmentPreview(
      PickedAttachment attachment, bool isImage, int index, bool isDark) {
    return Stack(
      children: [
        Container(
          width: 90,
            ...
            child: isImage
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text(
                    attachment.fileName, // sekarang nama file asli, bukan blob id!
                    maxLines: 2,
  ...
  // ─── Helpdesk Info Banner ─────────────────────────────────────────────────────

  /// Mapping kategori → info helpdesk (tanpa bergantung mock data)
  Map<String, dynamic> _getHelpdeskInfo(String category) {
    switch (category) {
      case 'Hardware':
      case 'Printer':
        return {
          'name': 'Helpdesk A', 'scope': 'Hardware & Printer',
          'color': const Color(0xFF1D4ED8), 'bg': const Color(0xFFEFF6FF),
          'icon': Icons.computer_outlined,
        };
      case 'Software':
      case 'Akun & Akses':
        return {
          'name': 'Helpdesk B', 'scope': 'Software & Akun & Akses',
          'color': const Color(0xFF166534), 'bg': const Color(0xFFF0FDF4),
          'icon': Icons.apps_outlined,
        };
      case 'Jaringan / Internet':
      case 'Email':
        return {
          'name': 'Helpdesk C', 'scope': 'Jaringan / Internet & Email',
          'color': const Color(0xFF6B21A8), 'bg': const Color(0xFFFAF5FF),
          'icon': Icons.wifi_outlined,
        };
      case 'Lainnya':
      default:
        return {
          'name': 'Admin', 'scope': 'Akan di-assign secara manual oleh Admin',
          'color': const Color(0xFF92400E), 'bg': const Color(0xFFFFF7ED),
          'icon': Icons.admin_panel_settings_outlined,
        };
    }
  }

  Widget _buildHelpdeskInfoBanner(bool isDark) {
    final info = _getHelpdeskInfo(_selectedCategory!);
    final textColor  = info['color'] as Color;
    final bannerColor = info['bg'] as Color;
    final helpdeskName = info['name'] as String;
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: isDark ? AppColors.cardDark : bannerColor,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isDark ? const Color(0xFF334155) : textColor.withOpacity(0.25),
        ),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Akan ditangani oleh $helpdeskName',
                  style: TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 13,
                    fontWeight: FontWeight.w700,
