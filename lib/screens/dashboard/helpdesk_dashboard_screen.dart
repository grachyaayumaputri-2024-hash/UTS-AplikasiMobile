...
class _HelpdeskDashboardScreenState extends State<HelpdeskDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.of(context).pushNamed(AppRoutes.createTicket),
        backgroundColor: const Color(0xFF0F766E),
        foregroundColor: Colors.white,
        icon: const Icon(Icons.add_rounded),
        label: Text('Buat Tiket', style: GoogleFonts.poppins(fontWeight: FontWeight.w600)),
      ),
    );
  }
                        ...
                        Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                _getScopeLabel(auth.currentUser?.id ?? ''),
                                style: GoogleFonts.poppins(
                                  fontSize: 10,
  ...
  Widget _buildMyTicketsSection() {
    return Consumer<DashboardProvider>(
      builder: (_, dash, __) {
        ...
        final myTickets = dash.myAssignedTickets
  ...
  Widget _buildAllOpenSection() {
    return Consumer<DashboardProvider>(
      builder: (_, dash, __) {
        final openTickets = dash.unassignedOpenTickets; // langsung dari backend, sudah sesuai scope kategori
  ...
  String _getScopeLabel(String userId) {
    // Scope ditentukan dari nama user yang login
    final auth = context.read<AuthProvider>();
    final name = auth.currentUser?.name.toLowerCase() ?? '';
    if (name.contains('helpdesk a') || name == 'helpdeska') return '💻  Hardware & Printer';
    if (name.contains('helpdesk b') || name == 'helpdeskb') return '🖥  Software & Akun';
    if (name.contains('helpdesk c') || name == 'helpdeskc') return '📡  Jaringan & Email';
    // Fallback berdasarkan email
    final email = auth.currentUser?.email.toLowerCase() ?? '';
    if (email.contains('helpdeska')) return '💻  Hardware & Printer';
    if (email.contains('helpdeskb')) return '🖥  Software & Akun';
    if (email.contains('helpdeskc')) return '📡  Jaringan & Email';
    return '🛠  Helpdesk';
  }
